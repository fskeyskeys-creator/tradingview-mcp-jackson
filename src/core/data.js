  ...
  const results = await evaluate(`
    (function() {
      try {
        ...
        // s.id is a method on these internal source objects, not a plain string property.
        function sid(s) { return typeof s.id === 'function' ? s.id() : s.id; }
        ...
        var available = strategySources.map(function(s) {
          ...
          return { entity_id: sid(s), name: name };
        });
        var strat = null;
        var wantId = ${entity_id ? JSON.stringify(entity_id) : 'null'};
        if (wantId) {
          for (var j = 0; j < strategySources.length; j++) { if (sid(strategySources[j]) === wantId) { strat = strategySources[j]; break; } }
        ...
        function flattenScalars(obj, out) {
          ...
          for (var k = 0; k < keys.length; k++) {
            var val = obj[keys[k]];
            var t = typeof val;
            if (val === null || val === undefined || t === 'function') continue;
            if (t === 'number' || t === 'string' || t === 'boolean') out[keys[k]] = val;
            else if (t === 'object' && !Array.isArray(val)) { out[keys[k]] = {}; flattenScalars(val, out[keys[k]]); }
            // arrays (trades, filledOrders, buyHold, marginUsage, etc.) are skipped here —
            // use pine_get_trades / data_get_trades or data_get_equity for those.
          }
        }
        var metrics = {};
